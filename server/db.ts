...
import { InsertUser, users, categories, content } from "../drizzle/schema";
 ...
 */
export async function getContentByCategory(categoryId: number) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    const result = await db
      .select()
      .from(content)
      .where(and(eq(content.categoryId, categoryId), eq(content.isActive, true)))
      .orderBy(content.createdAt);
    return result;
  } catch (error) {
    console.error('[Database] Failed to get content by category:', error);
    throw error;
  }
}
...
export async function getContentById(contentId: number) {
  ...
  try {
    ...
    console.error('[Database] Failed to get content by ID:', error);
    throw error;
  }
}
